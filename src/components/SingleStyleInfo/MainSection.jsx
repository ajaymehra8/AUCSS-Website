...
const MainSection = ({ details }) => {
  ...
  return (
    <div className="min-h-screen bg-[#1a1a28] py-[50px] px-[30px] lg:px-[150px] md:px-[70px] sm:px-[50px]">
      ...
      <pre className="relative mt-3 bg-black/30 rounded-lg p-4 text-sm text-gray-300 font-mono overflow-x-auto">
  {/* Copy Button */}
  <button
    onClick={copyToClipboard}
    type="button"
    className="p-2 hover:bg-white/20 rounded-lg transition-all duration-200 group absolute top-2 right-2"
    title="Copy code"
  >
    {copy ? (
  ...
  <div className="overflow-x-auto cursor-pointer">
    <p className="whitespace-pre">{details.code}</p>
  </div>
</pre>
