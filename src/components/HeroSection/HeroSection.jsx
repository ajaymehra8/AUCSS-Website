...
function HeroSection() {
  ...
  let filteredStyles =allStyles?.filter((snippet) => {
    ...
    const matchesLanguage =
      !selectedStyle || snippet.title.toLowerCase() === selectedStyle.toLowerCase();

    return matchesSearch && matchesLanguage;
  });
filteredStyles=filteredStyles.slice(0,6);
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-12">
        ...
        >
          <AnimatePresence mode="popLayout">
            {filteredStyles.length>0?filteredStyles.map((snippet) => (
              <SnippetCard key={snippet._id} snippet={snippet} />
            )): (<div className="flex justify-center items-center min-h-[50vh] w-[95vw]">
